...
def get_legal_ai_moves(ai_model, board, top_n=5):
    # get legal moves
    ...
    target_probabilities = ai_model.predict(tensor).reshape(-1)
...
def get_move_by_material(ai_model, board, number_of_top_moves=100, depth=1, depth_limit=3, alpha=-100000.0,
                         beta=100000.0):
    # if only want model moves
    if depth_limit == 1:
        return get_legal_ai_moves(ai_model, board, 1)[0]
    ...
    white_material = np.sum([pieces_score[str(p).lower()] for p in piece_map.values() if str(p).isupper()])
    black_material = np.sum([pieces_score[str(p).lower()] for p in piece_map.values() if str(p).islower()])
    ...
    if depth <= 1:
        top_n = 2
        prioritized_moves = get_legal_ai_moves(ai_model, board, top_n)
    ...
    for move, low_depth_limit, priority in prioritized_moves:
        ...
        mat_score = get_move_by_material(ai_model,
                                         future_board,
                                         depth=depth + 1,
...
def engine_move(ai_model, board):
    # move = np.random.choice(legal_move_probabilities[:3])
    s = time()
    move = get_move_by_material(
        ai_model,
        deepcopy(board),
        depth_limit=3,
        # no_depth=True
...
def play_game():
    ai_model = tf.keras.models.load_model("models/epochs_40_batch_size_512_2021-03-19_19:44:22.336811.h5")
    board = chess.Board()
    turn_number = 0
